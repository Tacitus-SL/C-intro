...
- [8. Многопоточность и синхронизация](#8-многопоточность-и-синхронизация)
  ...
  - [Condition Variable](#84-condition-variable)
  - [Barrier](#85-barrier)
...
- [13. Makefile](#13-makefile-продвинутый-уровень)
  ...
  - [Специальные переменные](#132-специальные-переменные)
...
- [14. Динамическая загрузка и сторонние библиотеки](#14-динамическая-загрузка-и-сторонние-библиотеки)
  - [Способы установки сторонних библиотек](#141-способы-установки-сторонних-библиотек)
  - [Линковка и `pkg-config`](#142-линковка-и-pkg-config)
  - [Динамическая загрузка библиотек](#143-динамическая-загрузка-библиотек)
- [15. Макросы v.2 и variadic functions](#15-макросы-и-variadic-functions)
  - [Предопределенные макросы](#151-предопределенные-макросы)
  - [Виды макросов](#152-виды-макросов)
    - [Object-like macros](#1521-object-like-macros-без-аргументов)
    - [Function-like macros](#1522-function-like-macros)
    - [Диагностические макросы](#1523-диагностические-макросы)
    - [Многострочные макросы](#1524-многострочные-макросы)
    - [Variadic macros](#1525-variadic-macros-макросы-с-переменным-числом-аргументов)
    - [Макросы, вызывающие макросы](#1526-макросы-вызывающие-макросы)
    - [X-Macro паттерн](#1527-x-macro-паттерн)
    - [Сравнительная таблица](#1528-сравнительная-таблица)
...
- [16. Секции и тестирование](#16-секции-и-тестирование)
  - [Секции](#161-секции)
  - [Тестирование](#162-тестирование)
    - [CUnit](#1621-cunit)
    - [Unity](#1622-unity)
    - [!!!Criterion](#1623-criterion)
    - [Check](#1624-check)
...
## 14. Динамическая загрузка и сторонние библиотеки
**Сторонние библиотеки** — это код, написанный другими разработчиками, который вы включаете в свой проект. Это требует решения трех вопросов: как установить, как скомпилировать с ними проект и как удалить.

---

### 14.1 Способы установки сторонних библиотек:
| Способ | Краткое описание | Как использовать | Удаление |
|--------|-----------------|-----------------|----------|
| Сборка из исходников | Компиляция из исходного кода с помощью Make/CMake | 1. `./configure && make && sudo make install`<br>2. По умолчанию устанавливается в `/usr/local` | Удалить установленные файлы вручную из `/usr/local/bin`, `/usr/local/lib`, `/usr/local/include` или использовать `sudo make uninstall`, если поддерживается |
| Однозаголовочные | Вся библиотека в одном `.h` файле | Скомпилировать `.h` в проект (например, `third_party`) и использовать флаг `-I` в Makefile | Просто удалить файл `.h` из проекта |
| Пакетный менеджер (apt) | Самый простой способ для Ubuntu/Debian | `sudo apt install <pkg>-dev` | `sudo apt remove <pkg>-dev` |

**Пример однозаголовочной библиотеки:**
Подключение:
```c
...
```make
CC = gcc
CFLAGS = -Wall -Wextra -O2 -Ithird_party

all: demo

demo: main.c third_party/mylib/mylib.h
    $(CC) $(CFLAGS) main.c -o $@

clean:
    rm -f demo
```
Иногда нужно определить реализацию макросом в одном `.c` файле:
```c
...
### 14.2 Линковка и `pkg-config`
Чтобы скомпилировать программу с библиотекой, компилятору нужно знать, где искать заголовочные файлы (`-I`) и где искать файлы библиотек (`-L<путь>` и `-l<имя>`).

`pkg-config` - это инструмент, который автоматически предоставляет необходимые флаги компилятора и линковщика для установленной библиотеки.

**Пример Makefile с `pkg-config`:**
```make
CC=gcc
CFLAGS=-Wall -Wextra -O2
LDFLAGS=

# 1. Используем pkg-config, чтобы получить флаги для библиотеки opencv4
OPENCV_FLAGS = $(shell pkg-config --cflags --libs opencv4)

all: myprog_opencv

myprog_opencv: myprog_opencv.o
	$(CC) $< -o $@ $(CFLAGS) $(LDFLAGS) **$(OPENCV_FLAGS)** # Передаем флаги
...
### 14.3 Динамическая загрузка библиотек
Это механизм, позволяющий загружать общие библиотеки (`.so`) непосредственно во время выполнения программы (после её запуска).

**Основные функции (требуется линковка с `-ldl`)**

| Функция | Назначение |
|---------|------------|
| `dlopen` | Загружает библиотеку (например, плагин) |
| `dlsym`  | Находит адрес символа (функции или переменной) в загруженной библиотеке |
| `dlclose` | Выгружает библиотеку |
...
// Символ "add" будет доступен для dlsym
```

---

```c
// main_dlopen.c (Загрузчик)
#include <dlfcn.h>
#include <stdio.h>
#include <stdlib.h>
// Обязательно используем typedef для указателя на функцию! [cite: 327]
typedef int (*add_fn_t)(int, int);

int main() {
	const char *so_path = "./libgreet.so";
	void *h = dlopen(so_path, RTLD_NOW); // Загружаем библиотеку [cite: 309]
	if (!h) {
		fprintf(stderr, "dlopen failed: %s\n", dlerror());
		return 1;
	}
	dlerror(); // Очистка ошибки перед dlsym [cite: 326]

	add_fn_t add_fn = (add_fn_t)dlsym(h, "add"); // Получаем адрес функции [cite: 311]
...
all: libgreet.so run_dynamic

# Сборка плагина (.so)
# -fPIC -shared: флаги для создания общей динамической библиотеки
libgreet.so: plugin.c
	$(CC) $(CFLAGS) -fPIC -shared $< -o $@ [cite: 303]

# Сборка загрузчика
# -ldl: линковка с библиотекой динамической загрузки (dlopen/dlsym) [cite: 344]
run_dynamic: main_dlopen.c
	$(CC) $(CFLAGS) $< -o $@ -ldl [cite: 303]

clean:
	rm -f libgreet.so run_dynamic
```

---

## 15. Макросы и variadic functions
Вспомним, что **макросы** — это механизм препроцессора, который выполняет текстовую подстановку до компиляции. Препроцессор не знает ничего о типах, функциях и синтаксисе C, следовательно нет типобезопасности, нет областей видимости, ошибки могут быть неочевидны и тд.

---
### 15.1 Предопределенные макросы
| Макрос             | Назначение               |
|--------------------|--------------------------|
...
| `__STDC_VERSION__` | Версия стандарта C       |

Расширения GCC / Clang:
 - __COUNTER__
 - __PRETTY_FUNCTION__
 - __BASE_FILE__

Использовать только при необходимости отладки и диагностики и строго за `#ifdef`.

---

### 15.2 Виды макросов
#### 15.2.1 **Object-like macros** (без аргументов)
```c
...
Преимущество перед `const`:
 - можно использовать в #if
 - можно переопределять через -D

---

#### 15.2.2 **Function-like macros**
```c
#define SQUARE(x) ((x) * (x))
```
Это не функция, а текстовая подстановка.
Правила:
 - всегда заключать параметры в скобки
 - всегда заключать всё выражение в скобки
 - не передавать аргументы с побочными эффектами

Рекомендация:
Если возможно — использовать static inline функцию.

#### 15.2.3 **Диагностические макросы**
```c
#define TRACE() \
    fprintf(stderr, "[%s:%d:%s]\n", __FILE__, __LINE__, __func__)
```
Преимущества:
 - не надо вручную писать файл и строку
 - работает в любом месте
 - легко отключается

---

#### 15.2.4 **Многострочные макросы**
Макрос должен вести себя как один оператор. Поэтому важно использовать `do { } while (0)`.
```c
#define M3P1(x) do { \
    (x) *= 3; \
    (x) += 1; \
...
```
Это гарантирует:
 - корректную работу с `if/else`
 - корректную обработку `;`

---

#### 15.2.5 **Variadic macros** (макросы с переменным числом аргументов)
```c
...
```
`__VA_OPT__` вставляет запятую только если аргументы существуют.

---

#### 15.2.6 **Макросы, вызывающие макросы**
По простому подстановка внутри подстановки.
```c
#define LOG_INFO(fmt, ...) LOG("INFO", fmt, ##__VA_ARGS__)
```
Можно создавать «макросы высшего порядка», но:
 - рекурсия невозможна
 - отладка усложняется

---

#### 15.2.7 **X-Macro паттерн**
Идея: один список данных → несколько генераций кода
```c
#define MY_POINT(X) \
    X(int, x, %d) \
    X(int, y, %d)
...
Мощно, но снижает читаемость — использовать осознанно.

---

#### 15.2.8 Сравнительная таблица
| Механизм | Кратко: что это | Когда использовать | Плюсы | Минусы / ограничения | Типичные примеры |
|--------|------------------|-------------------|-------|----------------------|------------------|
| Object-like macro | Именованная текстовая подстановка без аргументов | Константы, флаги сборки, `#ifdef` | Нет накладных расходов, работает в `#if`, можно задавать через `-D` | Нет типа, нет области видимости | `#define PAGE_SIZE 4096` |
| Function-like macro | Макрос с аргументами, похожий на функцию | Малые выражения, compile-time код | Быстро, можно вставлять код | Нет типобезопасности, возможен double-eval | `#define SQUARE(x) ((x)*(x))` |
| Multi-line macro | Макрос, содержащий несколько операторов | `ASSERT`, `CHECK`, логика с `return` | Можно инжектить control flow | Легко сломать `if/else`, сложная отладка | `do { ... } while (0)` |
| Diagnostic macro | Макрос с `__FILE__`, `__LINE__`, `__func__` | Логи, трассировка, ошибки | Точное место вызова | Не заменяет полноценный логгер | `TRACE()` |
| Variadic macro | Макрос с переменным числом аргументов | Логгеры, `printf`-обёртки | Удобный синтаксис | Проблемы с запятыми, зависит от стандарта | `LOG(fmt, ...)` |
| Macro calling macro | Макрос, использующий другой макрос | Иерархия логов, конфигурация | Уменьшает дублирование | Сложно читать, рекурсия невозможна | `LOG_INFO → LOG` |
| Stringizing macro (`#`) | Превращает аргумент в строку | Логи, тесты, сообщения об ошибках | Видно исходный код выражения | Только текст, не значение | `#x` |
| Token-pasting macro (`##`) | Склеивает токены в идентификаторы | Генерация имён | Генерация кода | Усложняет чтение | `name##_##id` |
| X-Macro | Список данных + генерация кода | `enum`↔`string`, структуры, таблицы | Один источник истины | Нестандартно, трудно читать | `X(name)` |
| Variadic function | Функция с `...` | `printf`, обработка аргументов | Реальная функция, стек | Нет типобезопасности | `int sum(int, ...)` |
| static inline function | Inline-функция | Почти всегда вместо макроса | Типобезопасность, отладка | Может не быть inline | `static inline int sq(int)` |

---

### 15.3 Stringizing и token pasting
- **Stringizing (`#`)**
Выражение не вычисляется, а просто превращается в текст.
```c
#define AS_STRING(x) #x
AS_STRING(1 + 2 * 3)  // "1 + 2 * 3"
```
Используется для логов, сообщений об ошибках, тестов.
...
Склеивание имен
```c
#define MAKE_FUNCTION(p, n) void p##_##n()
MAKE_FUNCTION(global, get)
```
Раскроется в:
```c
void global_get();
...
### 15.4 Variadic functions
Пример: `printf`, `scanf`.
Для реализации используются `<stdarg.h>`:

| Макрос     | Назначение                         |
|------------|------------------------------------|
| `va_list`  | Тип для хранения списка аргументов |
| `va_start` | Инициализация списка аргументов    |
| `va_arg`   | Получение очередного аргумента     |
| `va_end`   | Завершение работы со списком       |

**Ограничения variadic functions:**
 - нет типобезопасности
 - порядок и типы аргументов известны только программисту
...
Следовательно:
```c
va_arg(ap, char); // ошибка
va_arg(ap, int);  // правильно
```

**Как понять, сколько аргументов передали:**
Способ 1 - **Sentinel value**
```c
sum_until_zero(10, 20, 30, 0);
```
`0` - признак конца.

Способ 2 - **Format string**
```c
printf("%d %f %s", i, d, s);
```
Формат говорит, сколько и каких аргументов будет.
...
- Smoke — быстрые проверки ("хотя бы запускается")
- Performance — скорость и ресурсы
- Fuzz — случайные входные данные

---

**Когда какие тесты использовать:**
- Unit — всегда, на каждый коммит
- Integration — границы (I/O, IPC, файлы, сеть)
- Smoke — всегда
- System — CI, nightly
- Regression — каждый баг → тест
- Performance — горячие места
- Fuzz — парсеры, CLI, протоколы
...
### 16.2.1 CUnit
Используется, когда нужен простой и предсказуемый вариант.

| Плюсы                     | Минусы                 |
|---------------------------|------------------------|
| простой                   | нет CLI                |
| есть в системных пакетах  | ручная регистрация     |
| богатые assert’ы          | нужен main()           |
...
| Плюсы                  | Минусы             |
|------------------------|--------------------|
| очень лёгкий           | нет suites         |
| подходит для embedded  | ручная регистрация |
| один файл              | нужен main()       |
...
| Плюсы                               | Минусы            |
|-------------------------------------|-------------------|
| автоматическое обнаружение тестов   | тяжелее остальных |
| main() не нужен                     |                   |
| CLI и отчёты                        |                   |
| фикстуры                            |                   |
...
| Плюсы                    | Минусы               |
|--------------------------|----------------------|
| классический POSIX-стиль | устаревший           |
|                          | странный синтаксис   |
|                          | мало assert’ов       |
|                          | ручная регистрация   |
