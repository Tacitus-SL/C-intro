...
| Что                      | `#define`                                       | `const`                                  | `typedef`                                      |
|--------------------------|--------------------------------------------------|-------------------------------------------|------------------------------------------------|
| Обрабатывается           | препроцессором                                   | компилятором                              | компилятором                                   |
| Тип                      | отсутствует (текстовая подстановка)              | есть строгий тип                           | создаёт псевдоним типа                         |
| Видимость в отладке      | нет                                              | есть                                      | да (как тип)                                   |
| Для чего используется    | макросы, константы, условная компиляция          | неизменяемые переменные                   | удобное именование типов                       |
| Проверка типов           | нет                                              | есть                                      | относится к типам, не к значениям              |
| Память                   | не занимает (подстановка в код)                  | может занимать (если переменная)          | не занимает (только алиас)                     |
| Можно использовать в...  | `#if`, `#ifdef`, `#ifndef`                       | нельзя                                    | нельзя                                         |
| Область применения       | любой фрагмент текста                            | переменные или параметры                  | объявления типов (`struct`, `pointer`, и др.)  |
| Модифицирует поведение   | да (замена текста)                               | нет — обычная переменная                  | нет — только переименование                    |
| Лучше всего подходит для | макросов, флагов, встроенных выражений           | констант со строгой типизацией            | удобства записи типов и структур               |
