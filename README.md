...
Использовать только при необходимости и за `#ifdef`.

---

### 15.2 Виды макросов
- **Object-like macros** (без аргументов)
```c
...
Макрос, который выглядит как оператор, должен быть одним оператором.
```c
#define M3P1(x) do { \
    (x) *= 3; \
    (x) += 1; \
...
- **Variadic macros** (макросы с переменным числом аргументов)
```c
...
**Два основных паттерна использования:**
1. Sentinel value
```c
